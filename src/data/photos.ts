 ...
 *
 * Hot-link strategy (MSN-2975 perf chunks 2 + 6):
 *   - HERO + HOMEPAGE_HERO: served self-hosted as AVIF (srcSet, Q50 s=4)
 *     + WebP (src fallback, Q70) variants generated from the Commons
 *     originals and committed under
 *     `public/photos/hero-{slug}-{640,1080,1920,3840}w.{avif,webp}`.
 *     Each entry carries `url` (1920w WebP, default fallback) +
 *     `srcSet` (4 AVIF widths). This removes the 9 Wikimedia CDN
 *     round-trips for the LCP image on every page and the homepage
 *     hero, freeing up the critical path for in-page work.
...
export type WikimediaPhoto = {
   ...
   *
   * Format: `"/path/img-640w.avif 640w, /path/img-1080w.avif 1080w, ..."`.
   *
 ...
 * the MSN-2975 chunk-6 pipeline:
 *   - WebP variants at Q70 (`/photos/hero-{slug}-{w}w.webp`)
 *     generated by cwebp -q 70 -resize W 0 -m 6 -af.
 *   - AVIF variants at Q50 s=4 (`/photos/hero-{slug}-{w}w.avif`)
 *     generated by avifenc -q 50 --qalpha 50 -s 4 -j 4.
 *
 * Chunk 6 (MSN-2975): LCP-mobile was 4.4s on Lighthouse mobile. The
 * AVIF variants are 6-9% smaller than the WebP Q70 variants at the
 * mobile-critical 640w and 1080w sizes, and 25-32% smaller than the
 * original WebP Q80 baseline (the "chunk 2" target). On a 640px-wide
 * phone with DPR=2, the browser picks `640w` from `sizes="100vw"`, so
 * shaving bytes off the 640w file directly reduces the LCP image
 * download on mobile.
 *
 * The descriptor unit `w` lets the browser pick the best variant from
 * the matching `sizes="100vw"` attribute on the consuming image.
 * Modern evergreen browsers (Chrome 85+, Firefox 93+, Safari 16+)
 * all support AVIF natively. Older browsers fall back to the
 * `url` (a 1920w WebP) used by the `<img>` tag.
 */
 ...
 *  Used as the `<img src>` fallback for browsers that don't honour
 *  `srcSet` (or don't support AVIF). The AVIF variants are served
 *  through `srcSet` by all current evergreen browsers. */
